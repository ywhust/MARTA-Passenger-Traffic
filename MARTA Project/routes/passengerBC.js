...
exports.getBreezeCardNums = function (req, res) {
    ...
    if (typeof username == 'undefined') {
        sSql = "SELECT * FROM Breezecard WHERE BreezecardNum NOT IN (Select C.BreezecardNum from Conflict AS C)";
    } else {
        sSql = "SELECT * FROM Breezecard WHERE BreezecardNum NOT IN (Select C.BreezecardNum from Conflict AS C) AND BelongsTo = '" + username + "'";
    }
...
exports.checkForNull = function (req, res) {
    ...
    var sSql = `Select M.Enterfare, T1.StartTime, T1.BreezecardNum, M.Name from Trip AS T1, (Select * from Station AS S where S.StopID IN (Select StartsAt from Trip AS T where T.EndsAt is NULL)) AS M where T1.StartsAt = M.StopID and T1.BreezecardNum IN (select BreezecardNum from Breezecard where BelongsTo = '${username}');`;
